...
public class Constant {
    ...
    public static final String STRING_HELP_1 = "一、功能介绍\n" +
            ...
            "可以为日记归类标签。其中只有日记文本是必须的。" +
            "主界面以时间倒序展示日记，可以无限下滑加载更多日记。" +
            "你也可以搜索指定日记、按时间范围展示特定日记。" +
            "最后，未写完的日记可以存储到草稿箱暂存，需要注意的是，只会为你暂存日记文本。\n";
    public static final String STRING_HELP_2 = "\n二、安全相关\n" +
            "“消消乐”使用本地存储，没有软件服务器，不需要网络传输，消消乐也建议你关闭本软件的联网权限。" +
            ...
            "这种安全问题不是“消消乐”的问题，而是Android本身的特性，Android本地数据库存储的特性。\n" +
    ...
    public static final String STRING_HELP_3 = "\n三、权限使用说明\n" +
            ...
            "但你就只能使用部分功能了。其实想想也知道，日记配图肯定要从手机相册读取，这就是读权限，" +
            ...
            "因为这两个不是作者申请的，可能是使用了别人的代码，别人代码中有用到这些。\n";
    public static final String STRING_HELP_4 = "\n四、登录有关\n" +
            "登陆密码请牢记，登录密码只有登陆后才能在设置里更改。" +
            "不要试图猜密码的方式来找回密码，猜错5次会受到一次制裁，第一次制裁时间为1分钟，" +
            "第n次制裁时间为n^3分钟，(制裁等级的n最大为10。当内部记录的n达到50时，" +
    ...
    public static final String STRING_HELP_6 = "\n六、Q&A\n" +
            ...
            "Q：为什么软件里面的大图不能保存？\n" +
            "A：查看大图组件使用了别人的现成代码，它本身就不支持。我也不想改他的代码。" +
            "如果你特别想存某张图，可以到Hibara\\Diary\\image目录下根据文件日期找原始文件，" +
            "虽然文件后缀名为.hibara。它本质还是图片，没有经过加密的，直接复制走后再修改后缀名即可。" +
            "如果是非日记配图的图，应该在Config目录下。\n" +
            "Q：我要怎么反映BUG？\n" +
            "A：本软件应该不会再更新的了，如果坚持想修BUG，" +
            "请在开源代码的地址那里反映(“关于”那里复制网址打开后，" +
            "点“Issues”，之后你应该看得懂的了)。\n" +
            "Q：为什么我从备份文件里恢复的日记没有图片？\n" +
            "A：因为备份文件不会携带图片，只会携带图片的存储地址，因为配图已经存在你能访问的外部存储当中，" +
            "将整个image目录下的文件复制到鑫设备的image目录下，恢复的日记即可正常显示配图，注意:你不能随意更改文件名。" +
            "Q：为什么开源代码的地址打不开？\n" +
            "A：国外网站，多刷新几次。\n";
    public static final String STRING_HELP_7 = "\n七、写在最后\n" +
            "1.本软件不会盗取你任何数据，下面有开源代码可查。同样的，" +
            "如果你在使用本软件的过程中，产生任的何损失与本作者无关。\n" +
            "2.更多软件功能并没有在这里写明，你可以自行发现，例如可以长按日记进行复制等。\n" +
            "3.软件兼容性，目前在Honor V10上使用正常，1080p屏幕使用应该不会有什么问题，其他分辨率设备没试。\n";
